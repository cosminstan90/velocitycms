#!/usr/bin/env bash
# deploy.sh — Zero-downtime deployment for Velocity CMS
#
# Usage:
#   ./deploy.sh              # deploy latest main branch
#   ./deploy.sh --skip-pull  # build from current working tree (useful in CI)
#
...
echo ""
echo "════════════════════════════════════════════"
echo "  Velocity CMS — Production Deploy"
...
bash ./backup-before-deploy.sh || {
  ...
  echo "   Use --skip-backup to override (not recommended)."
  exit 1
}
...
# ── 3. Pull base images ────────────────────────────────────────────────────────
echo "▶ Step 3/6 — Pulling upstream images (postgres, redis, nginx)..."
$COMPOSE pull --ignore-buildable postgres redis nginx certbot umami 2>/dev/null || true
...
# ── 6. Rolling restart of Next.js only (zero-downtime) ────────────────────────
...
until $COMPOSE exec -T nextjs wget -qO- http://localhost:3000/api/health >/dev/null 2>&1; do
  ...
  if [ $RETRIES -eq 0 ]; then
    echo "✗ Health check failed after 30s. Check logs: docker compose logs nextjs"
    exit 1
  fi
  sleep 2
done
